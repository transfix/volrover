...
//
//   C++ owns `app` --> EmbeddedInterpreter builds PyHost(app) --> boots CPython,
//   `import vrhost` (auto-imports pycvc, registering the shared SWIG app type),
//   capsule-binds the host --> a Python script calls
//   `pycvc.state_set(vrhost.host.app(), path, value)` --> we read that value back
//   out of the SAME C++ cvc::state tree.
//
// If vrhost minted its own incompatible app type (the failure the %import in
// vrhost.i prevents), state_set would reject the handle and this test would fail.
//
...
TEST_F(VrHostBindingTest, HostBoundAtBoot) {
  ...
  ASSERT_TRUE(interp->run_string_capture(
      "vrhost.host is not None and vrhost.app() is not None", out, err))
      << err;
  EXPECT_NE(out.find("True"), std::string::npos) << "out=[" << out << "] err=[" << err << "]";
}

// The capstone: a script writes state THROUGH vrhost.host.app(); C++ reads it
...
TEST_F(VrHostBindingTest, ScriptWritesLiveAppState) {
  ASSERT_TRUE(interp->run_string(
      "import vrhost, pycvc\n"
      "pycvc.state_set(vrhost.host.app(), 'volrover3.vrhost_demo', '99')\n"))
...
// on vrhost.host.app() must NOT be visible on a fresh, independent make_app(). The
...
TEST_F(VrHostBindingTest, HostAppIsDistinctFromMakeApp) {
  ASSERT_TRUE(interp->run_string(
      "import vrhost, pycvc\n"
      "pycvc.state_set(vrhost.host.app(), 'volrover3.iso_probe', 'host')\n"
      "other = pycvc.make_app()\n"
      "pycvc.state_set(vrhost.host.app(), 'volrover3.iso_result',\n"
